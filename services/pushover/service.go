...
func (s *Service) Alert(user, message, device, title, URL, URLTitle, sound string, level alert.Level) error {
	url, post, err := s.preparePost(user, message, device, title, URL, URLTitle, sound, level)
	if err != nil {
		return err
	}
...
func (s *Service) preparePost(user, message, device, title, URL, URLTitle, sound string, level alert.Level) (string, url.Values, error) {
	c := s.config()
